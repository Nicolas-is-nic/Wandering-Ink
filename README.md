...
1. 构建产出 `dist/proof-list.md`（当前 27 条全部 `proofed: false` 待校对）。
2. 将该文件交其他模型交叉校对：核对诗词原文逐字、系年、叙事史实。
3. 修订回填 YAML，改 `proofed: true`，重建。校对清单自动分组「待校对 / 已校对」。
...
- 诗词与系年未经逐字校对（全部 `proofed: false`），上线前必须完成交叉校对流程。
